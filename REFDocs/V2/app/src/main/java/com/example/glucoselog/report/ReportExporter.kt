...
object ReportExporter {
    fun exportMonthlyReport(
        ...
        val pageWidth = 1200
        val pageHeight = 1600
        val pageInfoFactory = { pageNumber: Int -> PdfDocument.PageInfo.Builder(pageWidth, pageHeight, pageNumber).create() }
        ...
        pages.forEachIndexed { index, pageEntries ->
            ...
            if (index == 0) {
                ...
                y += 28
                canvas.drawText("Avg CGM: ${"%.1f".format(summary.avgCgm)}   Avg Manual: ${"%.1f".format(summary.avgManual)}   Avg Diff: ${"%.1f".format(summary.avgDifference)}", 40f, y.toFloat(), bodyPaint)
                y += 28
                canvas.drawText("High/Low CGM: ${summary.maxCgm}/${summary.minCgm}   High/Low Manual: ${summary.maxManual}/${summary.minManual}", 40f, y.toFloat(), bodyPaint)
                y += 40
            } else {
                y += 24
            }
            ...
            canvas.drawText("Context", 350f, y.toFloat(), headerPaint)
            ...
            pageEntries.forEach { entry ->
                ...
                canvas.drawText(entry.contextTag, 350f, y.toFloat(), smallPaint)
                ...
                canvas.drawText((entry.cgmReading - entry.manualReading).toString(), 950f, y.toFloat(), bodyPaint)
                y += 30
                if (entry.note.isNotBlank()) {
                    canvas.drawText("Note: ${entry.note.take(80)}", 60f, y.toFloat(), smallPaint)
                    y += 24
                }
            }

            canvas.drawText("Page ${index + 1} of ${pages.size}", 1000f, (pageHeight - 40).toFloat(), smallPaint)
    ...
    fun last30DaysEntries(allEntries: List<GlucoseEntry>): List<GlucoseEntry> {
        val now = System.currentTimeMillis()
        val start = now - 30L * 24L * 60L * 60L * 1000L
        return allEntries.filter { it.timestamp in start..now }
    }

    fun entriesForMonth(allEntries: List<GlucoseEntry>, month: YearMonth): List<GlucoseEntry> {
        val zone = ZoneId.systemDefault()
        val start = month.atDay(1).atStartOfDay(zone).toInstant().toEpochMilli()
        val end = month.plusMonths(1).atDay(1).atStartOfDay(zone).toInstant().toEpochMilli() - 1
        return allEntries.filter { it.timestamp in start..end }
    }
}
